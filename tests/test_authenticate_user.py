...
async def test_authenticate_user_success():
    ...
    with patch("security.verify_password", return_value=True) as mock_verify:
        result = await security.authenticate_user(mock_db, "user@example.com", "correct_password")
...
async def test_authenticate_user_wrong_password():
    ...
    with patch("security.verify_password", return_value=False) as mock_verify:
        result = await security.authenticate_user(mock_db, "user@example.com", "wrong_password")
...
async def test_authenticate_user_not_found():
    ...
    result = await security.authenticate_user(mock_db, "nonexistent@example.com", "password123")
